...
// MAJOR and MINOR - will assign dynamically, minor number will start usally from the zero value.
	...
	if (IS_ERR(cl))
	{
        ...
        // loop for two minor devices	
	for(i=0;i<2;i++){
	cdev_init(&c_dev[i], &driver_fops);//Initalize the character device , and initialize file operations structure driver_fops
	c_dev[i].owner = THIS_MODULE;
        //cdev_add registers a device with the kernel, MKDEV- will holds the major and minor numbers and helps us to create the dev_t identifies for the           device
  ...
  dev_ret=device_create(cl,NULL,MKDEV(MAJOR(dev), i),NULL,"finalchar%d",i);// create dev file in /dev/mychar*, now user applicatins can  interact with kernel driver via a device file operations
